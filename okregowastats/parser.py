...
class Parser:
    ...
    def get_matches_links_list(self):
        ...
        for match in all_matches:
            match_link = match.find_all("div", {'class': 'season__game-action'})[0].find_all(
                'a', {'class': 'action'})[0]['href']
    ...
    def analyze_players(self):
        ...
        for player in self.squads:
            if player[1] not in seen:
                ...
                self.players.append((player[1], player[0], player_info[1], player_info[2], player_info[3],
    ...
    def get_events(self, soup, game):
        ...
        for report in reports:
            minute = report.find('div', {'class': 'action-time'}).text.strip()
            minute = minute[:-5]

            if minute.find("+") > 0:
                minute = minute[:minute.find("+")]

            event_type = report.find('div', {'class': 'action-icon'}).find('i')['class']
            action_name = report.find('div', {'class': 'action-name'})
            team = action_name.text[action_name.text.find(" z ") + 3:]
            player = action_name.find('a')['href'].strip()
            team = self.team_clean(team)
            if minute == '':
                minute = 0
            events.append((game, int(minute), event_type[0], player, self.teams[team]))
        return events
    ...
    @staticmethod
    def download_match_page(match_link, match_path):
    ...
    @staticmethod
    def download_player_page(player_link, player_path):
    ...
    @staticmethod
    def read_match_page_from_disk(match_path):
    ...
    @staticmethod
    def read_player_page_from_disk(player_path):
