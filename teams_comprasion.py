...
teams_scrap = [["https://www.laczynaspilka.pl/druzyna-sezon/hutnik-szczecin/208837.html", "https://www.laczynaspilka.pl/druzyna/hutnik-szczecin,208837.html", "team_a.db", "Hutnik"],["https://www.laczynaspilka.pl/druzyna-sezon/olimp-goscino/208829.html","https://www.laczynaspilka.pl/druzyna/olimp-goscino,208829.html", "team_b.db", "Olimp"]]
teams_output = []

for team_scrap in teams_scrap:
    # this_club = "https://www.laczynaspilka.pl/druzyna-sezon/stal-szczecin/159615.html" # 2016/17
    matches_link = team_scrap[0] # 2017/18
    this_club = team_scrap[1]
    short_name = team_scrap[3]

    result = requests.get(matches_link)
    c = result.content

    conn = sqlite3.connect(team_scrap[2])
    ...
    for game in games_links:
        ...
        for report in reports:
            ...
            if (minuta == ''):
                minuta = 0
            events.append((game, int(minuta), type[0], zawodnik, teams[team.strip()]))
    ...
    teams_output.append((scored_goals, conceded_goals, team_squad, short_name))


template = Template(open("template/match_report.html").read())
output = template.render(teams=teams_output)
...
f.close()
css = 'template/css/bootstrap.css'
pdfkit.from_string(output, 'out.pdf', css=css)
# strzelane gole
